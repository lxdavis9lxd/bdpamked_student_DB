...
exports.findOne = async (customerNumber,transactionNumber) => {
    const query = `SELECT  ce.customerName as customerNumber_Value, t.* FROM payments t  join customers ce on t.customerNumber = ce.customerNumber  WHERE t.customerNumber=? AND t.transactionNumber=? LIMIT 0,1`;
...
exports.search = async (offset, pageSize, key) => {
    const query = `SELECT  ce.customerName as customerNumber_Value, t.* FROM payments t  join customers ce on t.customerNumber = ce.customerNumber  WHERE  LOWER(t.customerNumber) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.transactionNumber) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.paymentDate) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.amount) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.createdBy) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.modifiedBy) LIKE `+SqlString.escape('%'+key+'%')+` LIMIT ?, ?`;
    return getRows(query,[offset, pageSize]);
}

exports.searchCount = async (key) => {
    const query = `SELECT count(*) TotalCount FROM payments t  join customers ce on t.customerNumber = ce.customerNumber  WHERE  LOWER(t.customerNumber) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.transactionNumber) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.paymentDate) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.amount) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.createdBy) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.modifiedBy) LIKE `+SqlString.escape('%'+key+'%')+` `;
    const result = await getRows(query);
    if (result && result[0] && result[0].TotalCount && result[0].TotalCount > 0) {
        return result[0].TotalCount;
    } else {
