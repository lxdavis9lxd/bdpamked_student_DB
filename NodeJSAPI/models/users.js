...
exports.search = async (offset, pageSize, key) => {
    const query = `SELECT  t.* FROM users t  WHERE  LOWER(t.id) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.Username) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.password) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.role) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.employeeNumber) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.createdBy) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.modifiedBy) LIKE `+SqlString.escape('%'+key+'%')+` LIMIT ?, ?`;
    return getRows(query,[offset, pageSize]);
}

exports.searchCount = async (key) => {
    const query = `SELECT count(*) TotalCount FROM users t  WHERE  LOWER(t.id) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.Username) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.password) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.role) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.employeeNumber) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.createdBy) LIKE `+SqlString.escape('%'+key+'%')+` OR LOWER(t.modifiedBy) LIKE `+SqlString.escape('%'+key+'%')+` `;
    const result = await getRows(query);
    if (result && result[0] && result[0].TotalCount && result[0].TotalCount > 0) {
        return result[0].TotalCount;
    } else {
